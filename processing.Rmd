...
df_ema <- ema %>%
  ...
  ungroup() %>%
  select(-c(StartDate, EndDate, Status, IPAddress, Progress, RecordedDate, ResponseId, RecipientLastName, RecipientFirstName, RecipientEmail, LocationLatitude, LocationLongitude, DistributionChannel, ExternalReference, `Duration (in seconds)`, Finished, UserLanguage)) %>%
    relocate(PID) %>%
    filter(PID != "000000")
```
